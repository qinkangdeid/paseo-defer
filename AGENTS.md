...
- `client/pill.tsx` owns the composer pill and its card. The card is the quick route: with something half-written in the prompt box it offers a chip per common wait, and a chip queues that text through `defer.create` without opening anything. Its own press must not reach the pill underneath — on web a press inside the card arrives there too — so anything the card does sets `cardPressedAt` first, and the component keeps the same guard between a chip and the card behind it. Both windows are `CARD_ECHO_MS`. Under the default `pillMode: "always"` it keeps one registration per *live session* — not per queue — because the pill is the plugin's only in-session UI. Under `pillMode: "waiting"` only a session with a queue carries one. The entrypoint therefore tracks the agent stream itself and must drop a registration when a session closes, is removed, or moves workspace.
...
| `check-pill.mjs` | The composer-pill entrypoint and its component: a pill per live session in each `pillMode`, the preview card's open/close/open-the-panel behaviour, sessions that close, are removed or move workspace, and every timer and subscription released on cleanup. It calls the component as a plain function against a fake React, so the card's own wiring is covered without a renderer. It also covers the composer draft travelling with each press that opens the panel, the card's quick-defer chips — including a prompt box emptied while the card was up — and the hover region reaching the host pill's edges. |
